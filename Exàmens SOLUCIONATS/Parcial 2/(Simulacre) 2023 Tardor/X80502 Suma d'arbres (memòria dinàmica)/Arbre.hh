...
Arbre suma (const Arbre& a) const{
  Arbre resultat;
  resultat.primer_node = sum_aux(primer_node, a.primer_node);
  return resultat;
}
...
static node_arbre* sum_aux(node_arbre* n,  node_arbre *m) {
  ...
  node_arbre* suma = new node_arbre;
  suma->info = n->info + m->info;
  suma->segE = sum_aux(n->segE, m->segE);
  suma->segD = sum_aux(n->segD, m->segD);
  return suma;
}
