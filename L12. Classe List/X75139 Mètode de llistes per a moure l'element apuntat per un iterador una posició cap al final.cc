...
public:
  ...
  void moveTowardsEnd(iterator &it) {
    Item *mogut = it.pitem;
    Item *no_mogut = mogut->next;
    if (mogut == itemsup.prev) return;
    //desconecto mogut
    mogut->prev->next = mogut->next;
    mogut->next->prev = mogut->prev;

    //modifico nodes mogut
    mogut->next = no_mogut->next;
    mogut->prev = no_mogut;

    //reconecto mogut
    no_mogut->next->prev = mogut;
    no_mogut->next = mogut;
  }
};
